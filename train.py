...
p = float(right)/found
r = float(right)/total
json.dump({'preds':preds,'p':p,'r':r,'f1':2*p*r/(p+r)},open(args.eval_result_filename,'w',encoding='utf-8'),ensure_ascii=False)
print('p='+str(p)+'\t'+str(r)+'\t'+str(2*p*r/(p+r)))

multi_queries = set(json.load(open('test_multi_questions.json','r',encoding='utf-8')))
test_data = [json.loads(line.strip()) for line in open('test_data_qg_attr_1023.jsonl','r',encoding='utf-8').readlines() if json.loads(line.strip())['source'] in multi_queries]
  ...
  for pred,d in zip(preds,test_data):
    ...
    if  pred_attr and d['entity'].strip()!='':
      ...
      for q in pred_attr.split(';'):
        # if q.strip() not in d['attr'].replace(' ','').split(';'):continue 
